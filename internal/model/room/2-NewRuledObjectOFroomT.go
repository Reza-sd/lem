...
func NewRuledObjectOFroomT(name rT, connectionSlice []rT, isEndroom bool) (*roomT, []errT) { //Constructor=factory function=builder
	...
	if err := r.set.name(name); err != nil {
		return nil, logger.Act.Err.Rlog(_CODE_NewRuledRoom_10, err, "set.name(name):", "name:", name)
	}
	if err := r.set.connectionSlice(connectionSlice); err != nil {
		return nil, logger.Act.Err.Rlog(_CODE_NewRuledRoom_20, err, "r.set.connectionSlice")
	}
	...
	if name == _START_ROOM_NAME || isEndroom {
		...
		if err := r.set.allSeats(_MAX_SEATS_START_END); err != nil {
			return nil, logger.Act.Err.Rlog(_CODE_NewRuledRoom_30, err, "r.set.allSeats")
		}
		if err := r.set.usedSeats(_USED_SEATS_START_END); err != nil {
			return nil, logger.Act.Err.Rlog(_CODE_NewRuledRoom_40, err, "r.set.usedSeats")
		}
	} else {
		if err := r.set.allSeats(_ALL_SEATS_NORMAL_ROOM); err != nil {
			return nil, logger.Act.Err.Rlog(_CODE_NewRuledRoom_50, err, "r.set.allSeats")
		}
		if err := r.set.usedSeats(0); err != nil {
			return nil, logger.Act.Err.Rlog(_CODE_NewRuledRoom_60, err, "r.set.usedSeats(0)")
		}
	}
